...
This page mirrors `src/medseg_architectures/models/unet.py` so readers can
inspect the complete compact implementation in one place. The main
[U-Net overview](../unet.md) keeps the guided walkthrough and curated code
excerpts close to the architecture explanation.

Keep this page synchronized with the source file when the implementation changes.

??? example "Full code: `src/medseg_architectures/models/unet.py`"
